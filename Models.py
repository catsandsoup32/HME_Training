...
class Model_1_Bool(nn.Module): # from https://actamachina.com/handwritten-mathematical-expression-recognition, CNN encoder and then transformer decoder
    def __init__(self, vocab_size, d_model, nhead, dim_FF, dropout, num_layers):
        super(Model_1_Bool, self).__init__()
        device = 'cuda'
    ...
    def decoder(self, features, tgt, tgt_mask):
        ...
        tgt = self.transformer_decoder(tgt=tgt, memory=features, tgt_mask=tgt_mask.to(torch.bool), tgt_key_padding_mask=padding_mask.to(torch.bool)) # type match
        #tgt = self.transformer_decoder(tgt=tgt, memory=features, tgt_mask=tgt_mask.to(torch.float32), tgt_is_causal=True) # type match
...
class DecoderOnly(nn.Module):
    ...
    def forward(self, src, tgt, tgt_mask):
        output = self.decoder(src, tgt, tgt_mask)
        return output
