...
def main(num_epochs, model_in, lr, experiment_num, use_test_in_train):
    ...
    model.load_state_dict(torch.load("runs/Exp16E20.pt", map_location=device, weights_only=True))
    ...
    for epoch in range(21, num_epochs+1):
        ...
        counter = 0
        with torch.no_grad():
            for images, tgt, reversed_tgt, tgt_mask in tqdm(val_loader, desc=f"Epoch {epoch}: Val Loop"):
                counter += 1
                images = images.to(device)
                tgt = tgt.to(device)
                reversed_tgt = reversed_tgt.to(device)
...
# Exp 14 starts with LR=5e-7 and synthetic set

# Exp 15 starts with 2e-6 and no synthetic set

# Exp 16 starts with 7e-6

# Exp 16 starts with 5e-6
